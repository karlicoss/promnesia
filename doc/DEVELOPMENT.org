...
- ~pytest --pyargs promnesia.tests.test_server~: run just one file (i.e. module) via pytest
...
: tox -e end2end -- -s -k 'firefox_gui and test_visits'  # only run one specific test against gui firefox
...
https://addons.mozilla.org/en-US/developers/addon/promnesia/edit
