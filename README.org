...
The server regularly processes various *data sources* like:
...
- create =config.py=
  ...
  The only required settings are:
    ...
    - =SOURCES=
      ...
      You can see the list of available sources with more documentation on each of them here: [[file:doc/SOURCES.org][SOURCES]].

      In addition, you can find out more by:
...
- =tabs=: making the extension work without an explicit user action (hhe extension is meant to be a passive assistant)
  Might be optional in the future.

# TODO tabs could be optional in the future?  
* More links
- [[file:doc/SOURCES.org][Documentation on specific link sources]]
