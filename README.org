...
- [optional] check the config
  ...
  This is useful to quickly check your config: run =promnesia demo --config /path/to/config.py=.
...
- run the indexer: =promnesia index --config /path/to/config.py=
  ...
  NOTE: you can put promensia config in your home directory, e.g. =~/.config/promnesia/config.py=. That way in the future, you can simly run =promnesia index=.
  
  See [[https://github.com/ActiveState/appdirs/blob/3fe6a83776843a46f20c2e5587afcffe05e03b39/appdirs.py#L187-L190][this]] if you're not sure what's the user config dir on your system.
