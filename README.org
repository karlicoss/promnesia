...
TOC: [[#motivation][Motivation]] |  [[#demos][*Demos*]] | [[#how-does-it-work][How does it work?]] | [[#install][*Install*]] | [[#try-it-out][Try it out]] | [[#glossary][Glossary]]
...
- server/backend (=promnesia server= command)
  ...
  # One TODO normalises/URL equivalence. TODO link to urls suck

- indexer (=promensia index= command)

  Indexer *indexes* data sources, extracting URLs and other useful information
  # TODO mention contexts here?

** Data sources
Promnesia comes with a variaty of builtin source. It supports:
...
Obviously, visiting pages in your browser results in visits, but in Promnesia it also captures links that you interacted with
in other applications and services.

In [[file:src/promnesia/common.py][code]], visits are reprented as =class Visit= (and =class DbVisit=).
...
- *timestamp*: when the page was visited
  ...
  Required, but in the future, will be optional (sometimes you don't have a meaningful timestamp).
...
- *locator*: what's the origin of the visit?
  ...
  - locators for a link extracted from Reddit point straight into Reddit posts and comments where they occured
  - locators for a link extracted a local file point straight into text files on your disk (and clicking on it will open your text editor via MIME integration!)

  Required, but in the future might be optional.
...
- *context*: what was the context, in which the visit occured?
  ...
  - context for a link from org-mode file is the whole outline, in which it occured
  ...
  However they are useful to have as they fill in the gaps and provide means of tracing through your history.

  Optional.

- *duration*: how long we have spent on the page
...
- [[file:src/promnesia/sources/takeout.py][Google Takeout]] indexer
  ...
  There isn't any context for visits from takeout, it's just a glorified database export.

- [[file:src/promnesia/sources/instapaper.py][Instapaper]] indexer

  Generates a visit for each highlight on a page:

  - *url*: original URL of the annotated page
  - *timestamp*: time when you created the highlight
  - *locator*: permalink to the highlight into Instapaper's web app
  - *context*: highlight body

- [[file:src/promnesia/sources/markdown.py][Markdown]] indexer

  Extracts any links it fined in Markdown files:

  - *url*: extracted link
  - *timestamp*: Markdown doesn't have a well defined
